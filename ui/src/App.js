...
function App() {
    ...
    function getRooms() {
        ...
        const todayString = new Date(dayRef.current.getTime()).toLocaleDateString(
            "en-CA",
        ); //gets date in yyyy-mm-dd format
        const endString = new Date(
            dayRef.current.getTime() + 15 * 24 * 60 * 60 * 1000,
        ).toLocaleDateString("en-CA");
        axios.get(
            `https://api.multibooker.raybb.dev/get-rooms?from_date_string=${todayString}&to_date_string=${endString}`,
        ).then((res) => {
            const out = {};
            ...
            toast.error(
                "Something went wrong while getting rooms! Refresh the page.",
            );
            console.log(err);
        });
    }
    ...
    return (
        <div
            className={"w-[100dvw] h-[100dvh] bg-sage-green relative overflow-hidden"}
        >
            {showAuthPanel && (
                <Auth
            ...
            <div
                className={"flex-auto flex flex-row mobile:flex-col-reverse flex-nowrap overflow-hidden w-full h-full"}
            >
                <div
                    className={"w-[22rem] min-w-[22rem] mobile:w-full mobile:h-1/2 overflow-hidden flex-none"}
                >
                    <SideBar
                        rooms={rooms}
                        roomID={roomID}
