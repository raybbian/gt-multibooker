    ...
    return (
        <div
            ...
            <div className={"flex flex-col gap-2"}>
                ...
                >
                    <input className={"w-full bg-cream border-2 border-black p-2 text-base"} type={"text"}
            ...
            <div className={"flex flex-col"}>
                {Object.keys(selections).sort().map((date) => (
                    <div key={date} className={"flex flex-col gap-2"}>
                        {selections[date].length > 0 &&
                            <p className={"text-base font-semibold mt-2"}>{new Date(date).toDateString()}</p>}
                        {selections[date].map((booking) => (
                            <div key={`${booking[0]}-${booking[1]}`}
                                 className={"flex flex-row justify-between w-full h-full border-2 border-black"}>
                                <div className={"h-full flex"}>
                                    <div className={"w-2 h-full bg-sakura-pink"}></div>
                                    <p className={"p-2 ml-2 text-sm"}>
                                        {getDateTimeFromIdx(booking[0], new Date(date)).toLocaleTimeString('en-US', {
                                            hour: "2-digit",
                                            minute: "2-digit"
                                        })} - {getDateTimeFromIdx(booking[1] + 1, new Date(date)).toLocaleTimeString('en-US', {
                                        hour: "2-digit",
                                        minute: "2-digit"
                                    })}
                                    </p>
