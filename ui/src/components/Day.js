...
export default function Day({rooms, roomID, date, selections, setSelections}) {
    ...
    return (
        <div className={"w-full h-full flex flex-col border-2 border-black bg-cream"}>
            <p className={"px-2 border-b-2 border-black"}>{date.toDateString()}</p>
            <div
                className={"w-full h-full grid grid-cols-4 grid-rows-[repeat(24,_minmax(0,_1fr))] place-items-stretch"}
